...
# image_path = r'\\nindsdirfs2\shares\LFMI\FMM\Shared\rallapallih\ScanData\20211202_Controls\AverageImages' + '\\'
# atlas_path = r'\\nindsdirfs2\shares\LFMI\FMM\Shared\rallapallih\ScanData\20211202_Controls\Segmentation' + '\\'

image_path = r'\\nindsdirfs2\shares\LFMI\FMM\Shared\rallapallih\ScanData\PCP2CRE\Raw\20190523\AverageImages' + '\\'
atlas_path = r'\\nindsdirfs2\shares\LFMI\FMM\Shared\rallapallih\ScanData\PCP2CRE\segmentations\20190523' + '\\'
...
with open(r'\\nindsdirfs2\shares\LFMI\FMM\Shared\rallapallih\ScanData\PCP2CRE\Results\INTENSITY_RESULTS_'+ timestamp + '.csv', 'a', newline='') as csvfile:
# with open(r'\\nindsdirfs2\shares\LFMI\FMM\Shared\rallapallih\ScanData\20211202_Controls\Results\INTENSITY_RESULTS_'+ timestamp + '.csv', 'a', newline='') as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames, delimiter = ',')
    writer.writeheader()

    for file in glob.glob(image_path+"*.nii.gz"):
	
        mouseID = os.path.basename(file)[:-11]
        map_name = file
        map = nib.load((file))
        map_data = np.asarray(map.get_fdata(), dtype = np.float32)
        sizer = np.asarray(map.shape)
		
        atlas_PCL = nib.load((atlas_path + mouseID + '_PCL_Labels.nii.gz'))
        atlas_WM = nib.load((atlas_path + mouseID + '_WM_Labels.nii.gz'))  
        # atlas_Cortex = nib.load((atlas_path + mouseID + '_Cortex_Labels.nii'))
        atlas_IGL = nib.load((atlas_path + mouseID + '_IGL_Labels.nii.gz'))        
        atlas_ML = nib.load((atlas_path + mouseID + '_ML_Labels.nii.gz'))
        ...
        # dat_WM = map_data[atlas_data_WM]
        # dat_Cortex = map_data[atlas_data_Cortex]
