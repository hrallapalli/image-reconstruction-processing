...
#    print phaseorder    
    ...
    a = np.array([2 * fidnum * phasenum * slicenum* nechoes* nreps], dtype = np.int32)
...
#    slicenum = int(math.floor(np.size(a)/(fidnum*phasenum*nechoes*2)))
#    print(slicenum)
    
    sliceoffset = int(slicenum*(sliceoffset[0]/slicefov))
    print(sliceoffset)
    
    a.resize((fidnum * phasenum * slicenum * nechoes *nreps),2)
...
#    a=np.reshape(acomplex[0:fidnum*phasenum*slicenum],(fidnum,phasenum,slicenum), order = 'F')
    a=np.reshape(acomplex,(readnum,nechoes,ncoils,phasenum,slicenum,nreps), order = 'F')
...
#    atrain1 = zeros(readnum, ncoils, phasenum, slicenum)
    atranspose = np.transpose(a,(0, 3, 4, 1, 2, 5))
...
#    afft = np.fft.fftn(atranspose,axes=(0, 1, 2))
    ...
    plt.imshow(b[:,:,slicenum//2,0,0], cmap='gray',vmax = np.max(b[:,:,0,0,0]))
    plt.pause(.001)
 #   plt.draw()
