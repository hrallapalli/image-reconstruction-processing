...
directorybase = tkFileDialog.askdirectory(initialdir = '/home/doddst/data/20190607_104708_HM_A1145_2018_repeat_1_1')
...
#    atrain1 = zeros(readnum, ncoils, phasenum, slicenum)
    ...
    np.fft.fftshift(a,axes=(0,2,3))
    afft = np.fft.fftshift(np.fft.fftn(a[:,:,:,:],axes=(0,2,3)),axes=(0,2,3))
    ...
    afftroll = np.roll(afft,-phaseoffset,axis = 2);
    afft = np.roll(afftroll,sliceshift,axis = 3);         

    b=(abs(afft))
    
    plt.imshow(b[:,0,:,slicenum/2], cmap='gray',vmax = 0.8*np.max(b[:,0,:,slicenum/2]))
    plt.pause(.001)
 #   plt.draw()
